...
def calculate_percentage_coordinates(coordinates, image_width, image_height):
    if coordinates:
        ...
        y_percentage = 100 - ((coordinates["y"] / image_height) * 100)
...
if coordinates is not None and st.session_state.coordinates is None:
    ...
    st.rerun()
elif st.session_state.coordinates == coordinates and coordinates is not None and st.session_state.ignore is True:
    st.session_state.ignore = True
else:
    st.session_state.coordinates = coordinates
    st.session_state.ignore = False
